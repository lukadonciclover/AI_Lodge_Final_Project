...
function ThesisForm({ analysis }: { analysis: NonNullable<ReturnType<ReturnType<typeof useAnalyses>["getAnalysis"]>> }) {
  ...
  function save() { saveAnalysis({ ...analysis, thesis: { ...thesis, catalysts: thesis.catalysts.filter(Boolean), risks: thesis.risks.filter(Boolean) }, updatedAt: new Date().toISOString() }); setSaved(true); }
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <div className="space-y-5">
          <Card className="xl:sticky xl:top-24"><CardHeader className="border-b bg-navy-950 text-white"><CardTitle className="flex items-center gap-2 text-white"><Target size={17} className="text-blue-300" />Recommendation</CardTitle><p className="text-xs text-slate-400">Set your rating and valuation target.</p></CardHeader><CardContent className="space-y-5 pt-5">
            ...
            <div><Label htmlFor="targetPrice">Target price ({analysis.currency})</Label><Input id="targetPrice" type="number" min="0" step="any" value={thesis.targetPrice} onChange={(e) => { setThesis({ ...thesis, targetPrice: Number(e.target.value) }); setSaved(false); }} /></div>
            ...
            <Button className="w-full" variant="accent" onClick={() => { save(); router.push(`/analysis/${analysis.id}`); }}>Save & view analysis</Button>
          </CardContent></Card>
        </div>
      </div>
    </>
