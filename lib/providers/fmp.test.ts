...
describe("FmpFinancialDataProvider", () => {
  ...
  it("imports all company datasets and normalizes the response", async () => {
    const fetchMock = vi.fn(async (input: string | URL) => {
      ...
      if (path.includes("/profile/")) return jsonResponse([{ symbol: "ACME", companyName: "Acme" }]);
      if (path.includes("/quote/")) return jsonResponse([{ symbol: "ACME", price: 10 }]);
      if (path.includes("/income-statement/")) return jsonResponse([{ calendarYear: "2025", revenue: 5_000_000 }]);
      if (path.includes("/balance-sheet-statement/")) return jsonResponse([{ calendarYear: "2025", totalDebt: 1_000_000 }]);
  ...
  it("rejects API error payloads returned with a successful HTTP status", async () => {
    ...
    await expect(provider.searchCompanies("Apple")).rejects.toMatchObject({ statusCode: 503, message: expect.stringContaining("authentication failed") });
  });
  ...
  it("returns a useful rate-limit error", async () => {
    ...
    await expect(provider.searchCompanies("Apple")).rejects.toMatchObject({ statusCode: 429, message: expect.stringContaining("limit reached") });
  });
});
